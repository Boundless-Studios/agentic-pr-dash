...
def test_path_reuse_does_not_mask_detached_pr(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(_reconcile_mod, "_read_marker", lambda p: {"pr": "999"})  # different PR
    monkeypatch.setattr(_reconcile_mod, "_current_branch", lambda p: "bou-new")    # different branch
    monkeypatch.setattr(github_api, "get_review_threads", lambda pr, cwd=None: [_thread()])
    monkeypatch.setattr(_reconcile_mod, "_pr_open_state",
...
def test_reconcile_only_surfaces_target_repo_pr(tmp_path, monkeypatch, capsys):
    ...
    sl.append("sess-X", pr=12, branch="b", worktree=str(tmp_path / "wtB"),
              repo="org/repoB")
    monkeypatch.setattr(_reconcile_mod, "_repo_slug", lambda cwd: "org/repoA")
    monkeypatch.setattr(_reconcile_mod, "_collect_owned_worktrees", lambda sid, cwd, pid: [])
    monkeypatch.setattr(_reconcile_mod, "_iter_worktree_paths", lambda cwd: iter([]))
    monkeypatch.setattr(github_api, "get_review_threads", lambda pr, cwd=None: [_thread()])
    ...
    monkeypatch.setattr(_reconcile_mod, "_pr_open_state", fake_state)
    mc.main(["reconcile-prs", "--session-id", "sess-X", "--cwd", str(tmp_path)])
    records = [json.loads(l) for l in capsys.readouterr().out.splitlines() if l.strip()]
