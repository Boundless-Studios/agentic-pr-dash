...
def _stop_gate_impl(args) -> int:
    ...
    interval = _mc._env_int("STOP_INTERVAL", 180)
    state = _mc._load_stop_state(cwd)
    now = time.time()
    last_pending = bool(state.get("fingerprint"))
    if interval > 0 and not last_pending and (now - float(state.get("ts", 0) or 0)) < interval:
        return 0
    _mc._save_stop_state(cwd, {**state, "ts": now})

    session_id = args.session_id or _mc._read_session_marker(cwd)
    if session_id:
        owned = _mc._owned_worktrees_across_roots(session_id, cwd)
    else:
        owned = [cwd]

    pending: list[tuple[str, str]] = []
    for worktree in owned:
        code, text = _mc._check_worktree(worktree, session_id, claim=False)
        if code == 10:
            pending.append((worktree, text))
        elif code == 0 and session_id:
            marker = _mc._read_marker(worktree) or {}
            if str(marker.get("pr", "")).isdigit():
                _mc._prune_stale_marker(worktree, marker, session_id)

    if session_id:
        detached = [r for r in _mc._detached_records_across_roots(session_id, cwd)
                    if _mc._record_has_blockers(r)]
        detached.sort(key=lambda r: (0 if r["p1"] else 1, -r["unresolved_threads"], r["pr"]))
        for r in detached:
            pending.append(_mc._detached_pending_entry(r))

    if not pending:
        if (not getattr(args, "no_waiter", False)) and session_id:
            worktree_prs = (
                set() if _mc._detached_loop_alive(cwd) else _mc._owned_open_pr_numbers(owned)
            )
            detached_prs = set()
            for _dr in _mc._detached_records_across_roots(session_id, cwd):
                if _dr.get("state") not in ("merged", "closed", "draft", "unknown"):
                    detached_prs.add(_dr["pr"])
            open_prs = worktree_prs | detached_prs
            if open_prs and not _mc._await_alive(cwd, session_id):
                ...
                _mc._save_stop_state(cwd, {"ts": now, "fingerprint": fingerprint, "count": count})
                threshold = _mc._env_int("STOP_LOOP_THRESHOLD", 3)
                if count < threshold:
                    print(_mc._build_waiter_block(open_prs, cwd, session_id), file=sys.stderr)
                    return 2
                _mc._save_stop_state(cwd, {"ts": now})
                return 0
        _mc._save_stop_state(cwd, {"ts": now})
        return 0

    fingerprint = _mc._stop_fingerprint(pending)
    count = int(state.get("count", 0)) + 1 if state.get("fingerprint") == fingerprint else 1
    _mc._save_stop_state(cwd, {"ts": now, "fingerprint": fingerprint, "count": count})

    print(_mc._build_stop_block(pending), file=sys.stderr)
